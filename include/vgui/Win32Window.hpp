...
public:
    ...
    {
        if(m_hwnd)
            throw ExceptionInfo << "Win32 window already created.";
        ...
        wndclassex.lpfnWndProc = static_cast<WNDPROC>(Win32Window::StaticProcessEvent);
...
private:
    ...
    static LRESULT CALLBACK StaticProcessEvent(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
    {
        ...
        if(window && window->ProcessEvent(handle, msg, wparam, lparam, result))
            return result;
        else
